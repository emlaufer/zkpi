...
#print bool.ff_ne_tt
#print bool.no_confusion
def bor_eq_combine {a b c: bool} (h : a = tt ∨ b = tt ∨ c = tt): a || b || c = tt :=
begin
  cases a,
  cases b,
  exact (or.elim h (λ p, absurd p bool.ff_ne_tt) (λ p, or.elim p (λ p, absurd p bool.ff_ne_tt) (bor_eq_left))),
...
begin
  cases a,
  exact (or.elim h (λ p, absurd p bool.ff_ne_tt) (λ p, bor_eq_left p)),
  exact bor_eq_right (eq.refl tt),
end
...
begin
  intros,
  exact or.elim h (or.intro_left (r ∨ q)) (λ qr, or.intro_right p (or.comm.mp qr)),
end

def mem_reorder : ∀ {x y z: ℕ} {tl : list ℕ} (h : mem x (y :: z :: tl) = tt), mem x (z :: y :: tl) = tt :=
begin
  intros,
  let h1 := bor_eq_split h,
  let h2 := or.elim h1 (λ p, or.intro_left (is_eq y x = tt) (bor_eq_split p)) (λ p, or.intro_right _ p),
  let h3 := or.assoc.mp h2,
  let h4 := or_comm_helper h3,
  let h5 := bor_eq_combine h4,
  exact h5,
end

def subset : list ℕ -> list ℕ -> Prop :=
  λ s l, list.rec_on s true (λ hd tl prev, (mem hd l = tt) ∧ prev)
...
begin
  intros,
  let h1 := and.elim_left (and.assoc.mpr h),
  let h2 := and.elim_right (and.assoc.mpr h),
  exact and.assoc.mp (and.intro (and.comm.mp h1) h2),
end
...
def sorted : list ℕ -> Prop :=
  λ l, list.rec_on l true (λ hd tl prev, list.rec_on tl true (λ hd_tl tl_tl prev_tl, le_eff hd hd_tl ∧ prev) )
...
#print or.elim
def mem_insert_sorted {x : ℕ} {l : list ℕ} (y : ℕ ) (h : mem x l = tt) : mem x (insert_sorted y l) = tt :=
begin
  induction l,
  dunfold mem at h,
  exact false.elim (bool.ff_ne_tt h),
  dunfold insert_sorted,
  cases (decidable_bool_eq (le_eff y l_hd)),
  let h_1 := not_eq_ff_eq_tt h_1,
  exact @eq.subst bool (λ b, mem x (bool.rec (l_hd :: list.rec [y] (λ (hd : ℕ) (tl prev : list ℕ), bool.rec (hd :: prev) (y :: hd :: tl) (le_eff y hd)) l_tl) (y :: l_hd :: l_tl) b) = tt) tt (le_eff y l_hd) (flip_eq h_1) ((mem_cons y h)),
  cases (decidable_bool_eq (is_eq l_hd x)),
  apply @eq.subst bool (λ b, mem x (bool.rec (l_hd :: list.rec [y] (λ (hd : ℕ) (tl prev : list ℕ), bool.rec (hd :: prev) (y :: hd :: tl) (le_eff y hd)) l_tl) (y :: l_hd :: l_tl) b) = tt) ff (le_eff y l_hd) (flip_eq h_1),
  dunfold mem,
  let h_2 := not_eq_ff_eq_tt h_2,
  exact (bor_eq_combine2 (or.intro_right _ h_2)),
  apply @eq.subst bool (λ b, mem x (bool.rec (l_hd :: list.rec [y] (λ (hd : ℕ) (tl prev : list ℕ), bool.rec (hd :: prev) (y :: hd :: tl) (le_eff y hd)) l_tl) (y :: l_hd :: l_tl) b) = tt) ff (le_eff y l_hd) (flip_eq h_1),
  dunfold mem,
  dunfold mem at h,
  let h := or.elim (bor_eq_split h) (λp, p) (λ p, false.elim (bool.ff_ne_tt (eq.trans (flip_eq h_2) p))),
  let h1 := or.intro_left (is_eq l_hd x) (l_ih h),
  let h2 := bor_eq_combine2 h1,
  exact h2,
end
...
begin
  ...
  let h_1 := not_eq_ff_eq_tt h_1,
  apply (and_true (mem x (list.rec_on (l_hd :: l_tl) [x] (λ (hd : ℕ) (tl prev : list ℕ), bool.rec_on (le_eff x hd) (hd :: prev) (x :: hd :: tl))) = tt)).mpr,
  apply @eq.subst bool (λ b, mem x (bool.rec (l_hd :: list.rec [x] (λ (hd : ℕ) (tl prev : list ℕ), bool.rec (hd :: prev) (x :: hd :: tl) (le_eff x hd)) l_tl) (x :: l_hd :: l_tl) b) = tt) tt (le_eff x l_hd) (flip_eq h_1),
  exact (mem_head x (l_hd :: l_tl)),
  apply (and_true (mem x (list.rec_on (l_hd :: l_tl) [x] (λ (hd : ℕ) (tl prev : list ℕ), bool.rec_on (le_eff x hd) (hd :: prev) (x :: hd :: tl))) = tt)).mpr,
  apply @eq.subst bool (λ b, mem x (bool.rec (l_hd :: list.rec [x] (λ (hd : ℕ) (tl prev : list ℕ), bool.rec (hd :: prev) (x :: hd :: tl) (le_eff x hd)) l_tl) (x :: l_hd :: l_tl) b) = tt) ff (le_eff x l_hd) (flip_eq h_1),
  let h1 := l_ih (subset_nil l_tl),
  let h1 := (and_true (mem x (list.rec_on l_tl [x] (λ (hd : ℕ) (tl prev : list ℕ), bool.rec_on (le_eff x hd) (hd :: prev) (x :: hd :: tl))) = tt)).mp h1,
